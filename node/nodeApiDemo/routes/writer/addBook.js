...
router.use("", async function(req, res, next) {
  ...
  try {
    let sql = `INSERT INTO book(name, author, description, reptileType, type,updateTime,bookType,bookStatus,isJin) VALUES ("${
      book.title
    }","${tool.toSql(book.author)}","${tool.toSql(book.description)}", ${
      book.reptileType
    }, ${book.type}, date_sub("${book.updateTime}",interval 0 day), "${
      book.bookType
    }", ${book.bookStatus},${book.isJin})`;
    await db.query(sql);
  } catch (err) {
